...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function toggleUserDataChangeMode(isChangeMode: boolean) {
    if (!(mainBtnsWrapper instanceof HTMLElement) || !(saveBtnsWrapper instanceof HTMLElement))
      return;
    if (isChangeMode) {
  ...
  function setDisabledForUserDataFields(fieldsWrapper: HTMLElement, disabled: boolean) {
  ...
  function handleMainUserData() {
    ...
    const data = fields.reduce((acc: Record<string, string>, item: ChildNode) => {
